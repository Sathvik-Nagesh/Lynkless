export class E2EEHelper {
  ...
  static async decryptFile(file: File, password?: string): Promise<File> {
    ...
    // Bolt: Use zero-copy views to pass data to crypto.subtle without full buffer copies.
    const salt = new Uint8Array(arrayBuffer, 0, 16);
    const iv = new Uint8Array(arrayBuffer, 16, 12);
    const encryptedData = new Uint8Array(arrayBuffer, 28); // Zero-copy view on the encrypted segment
