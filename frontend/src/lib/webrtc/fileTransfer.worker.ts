...
interface WorkerTransferState {
  ...
  // Bolt: Optimized header validation components
  exp0?: number;
  exp1?: number;
  exp2?: number;
  exp3?: number;
...
// Bolt: Single-slot cache for high-frequency write path
let lastWriteCache: {
  fileId: string;
...
async function handleMessage(e: MessageEvent) {
  ...
  if (msg.type === 'init') {
    ...
    try {
      ...
      // Bolt: Pre-calculate expected ID components for O(1) validation in hot path
      let exp0, exp1, exp2, exp3;
      if (binaryId) {
      ...
      // Seed cache for next write
      lastWriteCache = { fileId, accessHandle, meta };
    ...
    if (accessHandle && meta && chunkIndex !== undefined && data) {
      try {
        // Bolt: Optimized 4x uint32 header validation (O(1) CPU vs O(N) loop)
        if (meta.exp0 !== undefined) {
          ...
          // Fallback if exp components weren't initialized
          const expectedIdBuffer = fileIdBufferMap.get(msg.fileId);
          if (expectedIdBuffer) {
            const actualIdBuffer = new Uint8Array(data, 0, 16);
        ...
        const error = err instanceof Error ? err : new Error('Unknown worker write error');
