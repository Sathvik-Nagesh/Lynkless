...
interface WorkerTransferState {
  ...
  // Compact File ID components for ultra-fast 4-way uint32 comparison
  exp0: number;
  exp1: number;
  exp2: number;
  exp3: number;
...
async function handleMessage(e: MessageEvent) {
  ...
  if (msg.type === 'init') {
    ...
    try {
      ...
      // Bolt: Pre-calculate the four uint32 components of the binary file ID
      // This allows O(1) validation in handleMessage('write') via DataView.
      const idView = new DataView(new Uint8Array(binaryId).buffer);
      const exp0 = idView.getUint32(0, true);
      const exp1 = idView.getUint32(4, true);
      const exp2 = idView.getUint32(8, true);
      const exp3 = idView.getUint32(12, true);
    ...
    if (accessHandle && meta && chunkIndex !== undefined && data) {
      try {
        // Bolt: Fast Binary Header Validation (CPU Optimization)
        // Bypass compareUint8Arrays loop with four uint32 checks via DataView.
        ...
        if (view.getUint32(0, true) !== meta.exp0 ||
          ...
          // Still return the buffer for recycling if validation fails
