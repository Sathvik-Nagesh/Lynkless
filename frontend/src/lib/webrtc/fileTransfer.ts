...
export class FileTransferManager {
   ...
   * Bolt: Caps the pool at 64 slabs to prevent memory leaks while ensuring high reuse.
   */
  ...
  private async resumeSendFile(fileId: string, peerId: string, fromChunk: number): Promise<void> {
    ...
    // Bolt: Use Stream API for efficient reading and hoist getBinaryId
    ...
    try {
      while (true) {
        ...
        while (offset < value.length) {
          ...
          // Bolt: Use bufferPool for zero-allocation transmission
          ...
          // Return buffer to pool after it's copied into SCTP queue
  ...
  async sendFile(file: File, peerId: string): Promise<string> {
    ...
    // Bolt: Cache binary UUID for entire transfer (avoid per-chunk conversion)
    const binaryId = this.getBinaryId(fileId);
  ...
  async broadcastFile(file: File, peerIds: string[]): Promise<string> {
    ...
    // Bolt: Hoist getBinaryId for the entire broadcast session
    const binaryId = this.getBinaryId(fileId);
