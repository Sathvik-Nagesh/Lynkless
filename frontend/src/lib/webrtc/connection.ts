...
export class WebRTCManager {
  ...
  async sendToPeer(
    ...
    // Bolt Optimization: Replace O(N) array allocation (.filter + .reduce)
    // with a single-pass manual loop to select the best channel. This completely
    // eliminates GC pressure and per-chunk array allocations on the high-throughput path.
    ...
    // Fallback to legacy single data channel if no parallel channels are active/open
