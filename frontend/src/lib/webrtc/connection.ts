...
export class WebRTCManager {
  ...
  async sendToPeer(
    ...
    // Bolt: Zero-allocation round-robin / selection logic for parallel data channels.
    // Replaces filter() and reduce() to eliminate O(N) array allocation per chunk on the hot transfer path.
    ...
    // Fallback if no open channels were found in the dataChannels array
    if (!channel) {
