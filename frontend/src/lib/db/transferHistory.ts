...
export const getTransferStats = async () => {
  try {
    ...
    if (db) {
      ...
      // Bolt: Use a cursor to iterate through entries without loading the entire collection into memory.
      // This maintains O(1) space complexity and avoids memory spikes.
