export const compressImage = async (file: File, quality = 0.7): Promise<File> => {
  ...
  return new Promise((resolve) => {
    ...
    // to avoid expensive Base64 encoding/decoding and reduce memory pressure by ~33%.
    const objectUrl = URL.createObjectURL(file);
    const img = new Image();
    img.src = objectUrl;

    img.onload = () => {
      // Clean up the object URL immediately after the image has loaded into memory
