export const compressImage = async (file: File, quality = 0.7): Promise<File> => {
  ...
  return new Promise((resolve) => {
    // Bolt: Use URL.createObjectURL instead of FileReader to avoid Base64 overhead (CPU & ~33% memory)
    const imageUrl = URL.createObjectURL(file);
    const img = new Image();

    img.onload = () => {
      // Bolt: Immediate cleanup to prevent memory leaks
