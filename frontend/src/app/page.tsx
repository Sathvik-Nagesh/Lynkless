...
export default function Home() {
  ...
  // Derived state for showing file preview modal without triggering useEffect setState cascading renders
  const connectedPeersCountForPreview = useMemo(() => peers.filter(p => p.state === 'connected').length, [peers]);
  const showFilePreview = pendingFiles.length > 0 && connectedPeersCountForPreview > 0 && pendingFiles !== dismissedFiles;
  ...
  const handleConfirmSend = useCallback(async (password?: string, shouldZip?: boolean, compressImagesFlag?: boolean) => {
    // Clear pending files immediately so modal closes
      ...
      <AnimatePresence>
        {showFilePreview && (
          <FilePreviewModal
            ...
            onCancel={() => {
              setDismissedFiles(pendingFiles);
              setPendingFiles([]);
            }}
          />
        )}
