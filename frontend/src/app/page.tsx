...
export default function Home() {
  ...
  // Bolt: Derived state avoids cascading renders triggered by useEffect + setState
  ...
  const handleConfirmSend = useCallback(async (password?: string, shouldZip?: boolean, compressImagesFlag?: boolean) => {
    // Clear pending files immediately to close derived FilePreviewModal
  ...
  // Bolt: Derived state avoids cascading renders and keeps UI in sync with state
  ...
  const radarUsers = useMemo(() => {
    // Bolt: Use a Set for O(1) lookup in the map loop, optimizing from O(N*M) to O(N+M)
