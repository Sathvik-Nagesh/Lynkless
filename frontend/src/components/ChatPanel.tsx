...
/**
 * Memoized individual message item to prevent redundant re-renders
 */
const ChatMessageItem = memo(({ msg, showTimestamp, isSameSender, timeLabel }: ChatMessageItemProps) => (
  <div>
    {/* Time separator */}
    {showTimestamp && (
...
const ChatPanel = memo(function ChatPanel({ messages, onSendMessage, disabled, connectedPeers = [] }: ChatPanelProps) {
  ...
  // Bolt: Use lastReadCount state to derive unreadCount without useEffect cascading renders
  const [lastReadCount, setLastReadCount] = useState(messages.length);
  const [isTyping] = useState(false);
  const messagesEndRef = useRef<HTMLDivElement>(null);
  const inputRef = useRef<HTMLInputElement>(null);

  // Sync lastReadCount when expanded to keep unreadCount at 0
