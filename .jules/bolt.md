...
## 2026-06-25 - Zero-Allocation WebRTC Data Channel Selection
**Learning:** Performing array operations like `.filter()` and `.reduce()` inside high-frequency WebRTC `sendToPeer` calls (for every single 64KB chunk) creates significant CPU overhead, closure allocation, and garbage collection pressure on the main thread.
**Action:** Replace functional array pipelines in high-throughput hot paths with standard single-pass `for` loops to select assets (like data channels) with zero memory allocations and O(1) space complexity.
